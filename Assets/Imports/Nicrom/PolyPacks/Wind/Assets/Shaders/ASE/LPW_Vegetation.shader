...
Shader "Nicrom/LPW/ASE/Low Poly Vegetation"
{
    ...
    SubShader
    {
        ...
        void vertexDataFunc(inout appdata_full v, out Input o)
        {
            ...
            float4 AnimatedWorldNoise1344 = tex2Dlod(_NoiseTexture,
                                                     float4(
                                                         ((WorldSpaceUVs1638 * AnimatedNoiseTilling1639) + panner1643),
                                                         0, 0.0));
            float temp_output_1584_0 = radians(
                ((MB_WindDirection870 + (MB_WindDirectionOffset1373 * (-1.0 + ((AnimatedWorldNoise1344).r - 0.0) * (1.0
                    - -1.0) / (1.0 - 0.0)))) * -1.0));
            ...
            float4 StaticWorldNoise1340 = tex2Dlod(_NoiseTexture,
                                                   float4((WorldSpaceUVs1638 * StaticNoileTilling1640), 0, 0.0));
            ...
            float MB_RotationAngle97 = radians(
                ((((MB_Amplitude880 + (MB_AmplitudeOffset1356 * (StaticWorldNoise1340).r)) * sin(
                    (((objToWorld1649.x + objToWorld1649.z) + (_Time.y * (MB_Frequency873 + (MB_FrequencyOffset1474 * (
                        StaticWorldNoise1340).r)))) * MB_Phase1360))) + MB_DefaultBending877) * (ase_vertex3Pos.y /
                    MB_MaxHeight1335)));
            float3 appendResult1558 = (float3(0.0, ase_vertex3Pos.y, 0.0));
            float3 rotatedValue1567 = RotateAroundAxis(appendResult1558, ase_vertex3Pos, MB_RotationAxis1420,
                                                       MB_RotationAngle97);
            float3 rotatedValue1565 = RotateAroundAxis(float3(0, 0, 0), rotatedValue1567, MB_RotationAxis1420,
                                                       MB_RotationAngle97);
